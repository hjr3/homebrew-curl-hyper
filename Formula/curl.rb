class Curl < Formula
  ...
  def install
    ...
    # clang cannot find the path when homebrew is running ./configure
    # it does work when i manually run ./configure
    # leaving it out does not seem to cause a problem
    # LDFLAGS="-Wl,-rpath,#{Formula["hjr3/curl-hyper/libhyper"].opt_prefix}/target/release"
    args += %W[
      --with-hyper=#{Formula["hjr3/curl-hyper/libhyper"].opt_prefix}
    ]
    ### END PATCH
